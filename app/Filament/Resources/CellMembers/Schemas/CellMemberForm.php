        ...
        return $schema
            ->components([
                Select::make('member_id')
                    ...
                    ->getSearchResultsUsing(function (string $search) {
                        ...
                        try {
                            ...
                            Log::error('Member search error in CellMemberForm: ' . $e->getMessage());
                ...
                Select::make('cell_group_id')
                    ->label('Cell Group')
                    ->placeholder('Type to search for cell groups...')
                    ->options([]) // Start with empty options to force search-only behavior
                    ->searchable()
                    ->noSearchResultsMessage('No cell groups found. Try a different search term.')
                    ->loadingMessage('Searching cell groups...')
                    ->searchDebounce(300)
                    ->getSearchResultsUsing(function (string $search) {
                        if (strlen(trim($search)) < 1) {
                            return CellGroup::orderBy('name')->limit(20)->pluck('name', 'id')->toArray();
                        }
                        
                        try {
                            return CellGroup::where('name', 'LIKE', "%{$search}%")
                                ->orderBy('name')
                                ->limit(50)
                                ->pluck('name', 'id')
                                ->toArray();
                        } catch (\Exception $e) {
                            Log::error('Cell group search error in CellMemberForm: ' . $e->getMessage());
                            return CellGroup::orderBy('name')->limit(20)->pluck('name', 'id')->toArray();
                        }
                    })
                    ->getOptionLabelUsing(function ($value) {
                        if (!$value) return null;
                        $cellGroup = CellGroup::find($value);
                        return $cellGroup ? $cellGroup->name : null;
                    })
                    ->required(),
